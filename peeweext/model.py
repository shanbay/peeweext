...
class Model(pw.Model, metaclass=ModelMeta):
    created_at = DatetimeTZField(default=pendulum.now(pendulum.UTC))
    updated_at = DatetimeTZField(default=pendulum.now(pendulum.UTC))
...
def _touch_model(sender, instance, created):
    if issubclass(sender, Model):
        instance.updated_at = pendulum.now(pendulum.UTC)
